...
(function (_global) {
  ...
  var _configureTimerForStopwatch = function (timer, intervalFn) {
    timer.endTime = this.endTime ? _convertAnyTimeStringToArray(this.endTime) : null;
    timer.startTime = this.startTime ? _convertAnyTimeStringToArray(this.startTime) : [0];
  ...
  var Timer = function (timer, fnOrOptions, intervalFnArg) {
    ...
    if (typeof timer === 'object' && !Array.isArray(timer) ) _configureTimerForStopwatch(this);
    ...
    if (this._isStopwatch) {
      ...
      this.interval = setInterval(_mainStopwatchIntervalFn.bind(this), 1000);
